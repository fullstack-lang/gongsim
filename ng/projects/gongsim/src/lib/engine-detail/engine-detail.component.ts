...
// EngineDetailComponent is initilizaed from different routes
// EngineDetailComponentState detail different cases 
enum EngineDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class EngineDetailComponent implements OnInit {
	...
	state: EngineDetailComponentState = EngineDetailComponentState.CREATE_INSTANCE
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			...
			if (this.originStruct == undefined) {
				this.state = EngineDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getEngine(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case EngineDetailComponentState.CREATE_INSTANCE:
						this.engine = new (EngineDB)
						break;
					case EngineDetailComponentState.UPDATE_INSTANCE:
						let engine = frontRepo.Engines.get(this.id)
						console.assert(engine != undefined, "missing engine with id:" + this.id)
						this.engine = engine!
						break;
