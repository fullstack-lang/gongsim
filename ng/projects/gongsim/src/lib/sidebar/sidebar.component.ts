      ...
      this.frontRepo.GongsimCommands_array.forEach(
        gongsimcommandDB => {
          ...
          */
          ...
          nonInstanceNodeId = nonInstanceNodeId + 1
          gongsimcommandGongNodeInstance.children!.push(EngineGongNodeAssociation)

          /**
            * let append a node for the instance behind the asssociation Engine
            */
          if (gongsimcommandDB.Engine != undefined) {
            let gongsimcommandGongNodeInstance_Engine: GongNode = {
              name: gongsimcommandDB.Engine.Name,
              type: GongNodeType.INSTANCE,
              id: gongsimcommandDB.Engine.ID,
              uniqueIdPerStack: // godel numbering (thank you kurt)
                3 * getGongsimCommandUniqueID(gongsimcommandDB.ID)
                + 5 * getEngineUniqueID(gongsimcommandDB.Engine.ID),
              structName: "Engine",
              associationField: "",
              associatedStructName: "",
              children: new Array<GongNode>()
            }
            EngineGongNodeAssociation.children.push(gongsimcommandGongNodeInstance_Engine)
          }
