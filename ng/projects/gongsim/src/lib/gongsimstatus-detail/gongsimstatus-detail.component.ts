...
enum GongsimStatusDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			...
			if (this.originStruct == undefined) {
				this.state = GongsimStatusDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getGongsimStatus(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case GongsimStatusDetailComponentState.CREATE_INSTANCE:
